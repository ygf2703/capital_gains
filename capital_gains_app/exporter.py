...
def export_result(result: CalculationResult, output_path: str | Path) -> Path:
    ...
    wb = Workbook()
    wb.properties.creator = "Capital Gains"
    wb.properties.title = "Capital Gains FIFO Report"
    wb.properties.subject = "Capital gains analysis"
    wb.properties.description = f"Capital Gains desktop export generated by version {__version__}"
    dashboard = wb.active
...
def _format_sheet(ws) -> None:
    ...
    if ws.title == "Validation Issues":
        severity_colors = {"error": "4A1E26", "warning": "4A3D1C", "info": "1D2A39"}
        for row in range(2, ws.max_row + 1):
            severity = str(ws.cell(row=row, column=1).value or "").strip().lower()
